...
var evolution = {
	...
	display: function(array){ // displays seeds and also checks for a match to user phrase
		//alert('the world does not make sense');
		
		$('#results').html('<div><h2>your entry is: '+this.userPhrase+'</h2></div>')
	
		for(var i=0;i<array.length;i++){
	
			$('#results').append('<div><p>'+array[i].name+': '+array[i].value.join('')+'</p></div>')
			...
			$('#m_count').html(this.mutations)
			$('#g_count').html(this.generation +1)
		}
